...
function pushbulletCnt(content, accesstoken, device, device_num, redirect) {
  if (!content || content == "") {
    ui.toast({ text: "Clipboard is Empty", icon: "225", inset: 7 })
    delayClose()
  } else {
...
function toast(resp) {
  if (resp.response.statusCode == 400) {
    ...
    ui.toast({text:"Request Succeeded"})
    $ui.loading(false)
  } else {
...
function delayClose(notFresh=1) {
  ...
  $ui.render({
    ...
    views: [{
      ...
      },
      {
        ...
        props: {
          id: "message_tgpmbot",
          text: "You can send message to your Telegram by Push More Bot(https://t.me/pushmorebot)",
          font: $font(16),
          editable: 0
        },
        layout: function(make) {
      ...
      },
      {
        ...
        props: {
          id: "tgpmbot",
          placeholder: "Paste Your Telegram Push More Bot Webhook URL",
          align: $align.center,
          font: $font(15)
        },
        layout: function(make) {
          var preView = $("message_tgpmbot")
          make.top.equalTo(preView.bottom).inset(10)
          make.left.right.inset(10)
          make.height.equalTo(30)
        },
